...
class Cart extends Component {
    ...
    componentDidMount() {
        ...
        firebase.database().ref().child('cart').orderByChild('sessionId').equalTo(_sessionId).on('value', snap => {
            this.handleCartAmount(snap.val());
        });
    }

    handleCartAmount(data) {
        let _cart = Object.values(data),
            _quan = 0;
        _cart.map((item) => {
            _quan += item.quantity;
        });
        this.setState({cartQuan: _quan});
    }

    render() {
        return (
            <div className="cart">
                <svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="100%" height="100%" viewBox="8.3 0 105.5 126.1" preserveAspectRatio="xMinYMax meet">
                    ...
                    <text x="58.5" y="77" dy=".35em" textAnchor="middle" className="quantity" data-hook="cart-widget-items-count">{this.state.cartQuan}</text>
                </svg>
            </div>
        )
    }
